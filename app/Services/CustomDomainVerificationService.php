...
class CustomDomainVerificationService
{
     ...
     *
     * 1. Ownership - a TXT record at the domain's "_edunest-verify"
     ...
     * 2. Routing - the domain itself actually points at EduNest (a CNAME to
    ...
    public function verify(CustomDomain $domain): bool
    {
        ...
        if (! $this->routingRecordsMatchTarget($cnameRecords, $aRecords, $cnameTarget, $aRecordIp)) {
            $this->fail($domain, empty($cnameRecords) && empty($aRecords)
                ...
                : "Domain ownership was confirmed, but {$domain->domain} isn't pointing to EduNest yet. Make sure its CNAME record points to {$cnameTarget}.");
