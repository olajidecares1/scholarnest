...
class CbtTestDocumentExtractionService
{
    private const API_URL = 'https://api.anthropic.com/v1/messages';

    private const ANTHROPIC_VERSION = '2023-06-01';

    private const TOOL_NAME = 'record_extracted_questions';

    /**
     * Extract structured question data directly from a PDF (Claude reads the PDF natively).
     * Unlike the global CBT catalog's extractor, the subject/class context is already
     * known (the teacher picked it when creating the test), so this returns a flat
     * question list rather than grouping by exam body/year.
     *
     * @return array{questions: list<array<string, mixed>>}
     */
    public function extractFromPdf(string $absolutePath, string $subject, string $className): array
    {
        ...
        return $this->call([
            ...
            ['type' => 'text', 'text' => $this->instructions($subject, $className)],
        ]);
    }

    /**
     * Extract structured question data from plain text already pulled out of a .docx file.
     *
     * @return array{questions: list<array<string, mixed>>}
     */
    public function extractFromText(string $text, string $subject, string $className): array
    {
        return $this->call([
    ...
    private function call(array $contentBlocks): array
    {
        ...
        if (($payload['stop_reason'] ?? null) === 'max_tokens') {
            throw new RuntimeException('The document produced more output than the model could return in one pass. Try splitting it into smaller uploads.');
        }

        $toolUse = collect($payload['content'] ?? [])->firstWhere('type', 'tool_use');

        if (! $toolUse) {
            throw new RuntimeException('The model did not return structured question data.');
        }

        return $toolUse['input'];
    }

    private function instructions(string $subject, string $className): string
    {
        $lines = [
            "You are extracting {$subject} test/examination questions for class \"{$className}\" from a document for a Nigerian school's computer-based testing system.",
            'Extract every multiple-choice question in the document as a flat list, in the order they appear.',
    ...
    private function toolSchema(): array
    {
        return [
            'name' => self::TOOL_NAME,
            'description' => 'Record every question extracted from the document.',
            'input_schema' => [
                'type' => 'object',
                'required' => ['questions'],
                'properties' => [
