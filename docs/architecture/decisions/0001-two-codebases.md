...
A fresh Laravel 13 project was started in `C:\xampp\htdocs\EduNest`, which was
...
The `edunest` database already belonged to **this** application, which was
alive and well in `Documents\EduNest`.

The empty `htdocs\EduNest` folder was misleading: this project used to live
...
- `edunest` still holds 99 tables and all its data
- `Documents\EduNest` and `D:\EduNest` were never written to

The Laravel 13 experiment was then repointed at a separate database,
`edunest_v13`, so it could not collide.
...
description of the standard EduNest should meet, not an instruction to start
again.
...
- Work continues in `Documents\EduNest`, on the `dev` branch.
...
- `C:\xampp\htdocs\EduNest` holds an abandoned Laravel 13 skeleton and the
  `edunest_v13` database. Both can be deleted once confirmed unwanted.
- `D:\EduNest` is a stale copy and should be removed once GitHub is set up, so
  there is no chance of editing the wrong one.

## Related fix

The same investigation found that `public/storage` was a junction pointing at
`C:\xampp\htdocs\EduNest\storage\app\public` — the old location, which is now
