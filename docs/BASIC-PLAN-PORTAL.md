...
| ---- | ---------------------- |
| Standard | `greenfield.edunest.com` — its own subdomain |
...
```
  edunest.com/{32-character token}
                 |
...
```
edunest.com/6219db402a20f65b63358972bd5274cd
```
...
identical on every EduNest installation in the world.
...
2. It would leak information. Confirming a school exists but is "not available
   here" tells an outsider both that EduNest has that customer and roughly what
   they pay for.
...
`edunest.com/greenfield-college` puts a school slug at the root of the site, in
...
```php
'(?!(?:portal|login|...|edunest)$)[a-z0-9]+(?:-[a-z0-9]+)*'
```

The `(?: ... )$` grouping matters. Written as `(?!portal|login|...|edunest$)`
