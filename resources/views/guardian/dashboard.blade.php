            ...
            <div class="flex flex-wrap gap-2">
                @foreach ($children as $child)
                    <a
                        ...
                        class="btn rounded-full border px-4 py-2 text-sm font-semibold transition-all duration-200 {{ $child->is($activeChild) ? 'border-primary-600 bg-primary-600 text-white' : 'border-gray-200 bg-white text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200' }}"
                    >
                        {{ $child->fullName() }}
                    </a>
                @endforeach
