    ...
    <div class="space-y-6" x-data="{ addExamOpen: false }">
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800 lg:rounded-[10px]">
            ...
            <form method="POST" action="{{ route('super-admin.cbt.exam-bodies.subjects.update', $examBody) }}" class="mt-4">
                        ...
                        <div class="mt-4">
                            ...
                            <div class="mt-2 flex flex-wrap gap-3">
                                @foreach ($categorySubjects as $subject)
                                    <label class="flex items-center gap-2 rounded-[5px] border border-gray-200 px-3 py-1.5 text-sm text-gray-700 transition-colors duration-150 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-700/50 lg:rounded-[8px]">
                                        <input
                                            type="checkbox"
                                            name="subjects[]"
                                            value="{{ $subject->id }}"
                ...
                <button type="submit" class="mt-5 rounded-[5px] bg-primary-500 px-4 py-2 text-sm font-semibold text-white transition-all duration-300 ease-out hover:-translate-y-0.5 hover:bg-primary-600 hover:shadow-md lg:rounded-[10px]">Save Subjects</button>
            </form>
        </div>
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800 lg:rounded-[10px]">
            <div class="flex items-center justify-between border-b border-gray-100 p-6 dark:border-gray-700">
                ...
                <button
                    ...
                    class="flex items-center gap-2 rounded-[5px] bg-primary-500 px-4 py-2 text-sm font-semibold text-white transition-all duration-300 ease-out hover:-translate-y-0.5 hover:bg-primary-600 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:shadow-none lg:rounded-[10px]"
                >
            ...
            <div @click.outside="addExamOpen = false" class="w-full max-w-md rounded-[5px] bg-white p-6 dark:bg-gray-800 lg:rounded-[10px]">
                ...
                <form method="POST" action="{{ route('super-admin.cbt.exams.store', $examBody) }}" class="mt-4 space-y-4">
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        <button type="submit" class="rounded-[5px] bg-primary-500 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-600 lg:rounded-[10px]">Create Exam</button>
                    </div>
                </form>
            </div>
        </div>
