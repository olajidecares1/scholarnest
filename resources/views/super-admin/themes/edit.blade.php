            ...
            <form method="POST" action="{{ route('super-admin.themes.update') }}" class="mt-4 space-y-4">
                ...
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    @foreach ($presets as $key => $preset)
                        <label
                            class="flex cursor-pointer items-center gap-3 rounded-[5px] border-2 p-3 transition-all duration-200 lg:rounded-[10px]"
                ...
                <div class="rounded-[5px] border border-dashed border-gray-300 p-4 dark:border-gray-600 lg:rounded-[10px]">
                    ...
                    <div class="mt-3 flex items-center gap-3">
                        ...
                        <button
                            type="button"
                            class="rounded-[5px] px-4 py-2 text-sm font-bold text-white shadow-md lg:rounded-[10px]"
                            :style="{ backgroundColor: colors[selected] }"
                        >
                            Sample Button
                        </button>
            ...
            <div class="mt-4 flex items-center gap-4">
                ...
                <form method="POST" action="{{ route('super-admin.themes.logo.update') }}" enctype="multipart/form-data" class="flex flex-1 items-center gap-3">
                    ...
                    <input
                        ...
                        required
                        class="flex-1 rounded-[5px] border border-gray-300 bg-white py-2 px-3 text-sm text-gray-700 shadow-sm file:mr-3 file:rounded-[5px] file:border-0 file:bg-primary-50 file:px-3 file:py-1.5 file:text-sm file:font-semibold file:text-primary-700 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 lg:rounded-[10px]"
                    >
                    <button type="submit" class="rounded-[5px] bg-gray-900 px-4 py-2 text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-gray-800 dark:bg-gray-700 dark:hover:bg-gray-600 lg:rounded-[10px]">Upload</button>
                </form>
            </div>
        </div>
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800 lg:rounded-[10px]">
            ...
            <div class="mt-4 flex items-center gap-4">
                <span class="flex h-10 w-10 items-center justify-center rounded-[5px] border border-gray-200 bg-gray-50 p-1.5 dark:border-gray-700 dark:bg-gray-900 lg:rounded-[10px]">
                    <img
                        src="{{ $settings->favicon_path ? \Illuminate\Support\Facades\Storage::url($settings->favicon_path) : asset('favicon-32x32.png') }}"
                        alt="Current favicon"
                        class="h-full w-full object-contain"
                    >
                </span>
