        ...
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-[5px] border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800 lg:rounded-[10px]">
            ...
            @if ($school->is_active)
                <form method="POST" action="{{ route('super-admin.schools.deactivate', $school) }}" onsubmit="return confirm('Deactivate {{ $school->name }}? Their admins will lose access immediately.');">
                    @csrf
                    <button type="submit" class="rounded-[5px] border border-red-300 px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-900/20 lg:rounded-[10px]">Deactivate School</button>
                </form>
            @else
                <form method="POST" action="{{ route('super-admin.schools.activate', $school) }}">
                    @csrf
                    <button type="submit" class="rounded-[5px] border border-green-300 px-4 py-2 text-sm font-semibold text-green-700 hover:bg-green-50 dark:border-green-800 dark:text-green-400 dark:hover:bg-green-900/20 lg:rounded-[10px]">Activate School</button>
                </form>
            @endif
        </div>
