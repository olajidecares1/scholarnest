            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($schools as $school)
                            <tr>
                                <td class="px-5 py-3">
                                    ...
                                    <p class="text-xs text-gray-500">{{ $school->billing_email ?? '—' }}</p>
                                </td>
                                <td class="px-5 py-3 text-gray-600">{{ $school->activeSubscription?->plan?->name ?? 'No plan yet' }}</td>
                                <td class="px-5 py-3 text-gray-600">{{ $school->users->count() }}</td>
                                <td class="px-5 py-3 text-gray-600">{{ $school->created_at->format('M j, Y') }}</td>
                                <td class="px-5 py-3">
                                    @if ($school->is_active)
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-5 py-10 text-center text-sm text-gray-500">No schools found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
