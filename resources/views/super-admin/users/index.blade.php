    ...
    <div class="space-y-6">
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white shadow-sm lg:rounded-[10px]">
            <form method="GET" action="{{ route('super-admin.users.index') }}" class="flex flex-wrap items-center gap-3 p-4">
                ...
                <select name="role" class="rounded-[5px] border-gray-300 py-2 text-sm shadow-sm focus:border-primary-500 focus:ring-primary-500 lg:rounded-[10px]">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($users as $user)
                            <tr>
                                ...
                                <td class="px-5 py-3">
                                    <span class="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-semibold text-gray-700">{{ $user->role->label() }}</span>
                                </td>
                                <td class="px-5 py-3 text-gray-600">{{ $user->school->name ?? '—' }}</td>
                                <td class="px-5 py-3 text-gray-600">{{ $user->created_at->format('M j, Y') }}</td>
                                <td class="px-5 py-3">
                                    @if ($user->is_active)
                                ...
                                <td class="px-5 py-3">
                                    ...
                                    @elseif ($user->is_active)
                                        <form method="POST" action="{{ route('super-admin.users.deactivate', $user) }}" onsubmit="return confirm('Deactivate {{ $user->name }}?');">
                                            @csrf
                                            <button type="submit" class="rounded-[5px] border border-red-300 px-3 py-1.5 text-xs font-semibold text-red-700 hover:bg-red-50 lg:rounded-[10px]">Deactivate</button>
                                        </form>
                                    @else
                                        <form method="POST" action="{{ route('super-admin.users.activate', $user) }}">
                                            @csrf
                                            <button type="submit" class="rounded-[5px] border border-green-300 px-3 py-1.5 text-xs font-semibold text-green-700 hover:bg-green-50 lg:rounded-[10px]">Activate</button>
                                        </form>
                                    @endif
                                </td>
                            </tr>
