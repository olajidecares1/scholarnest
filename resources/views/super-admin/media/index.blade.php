                ...
                <div>
                    ...
                    <div class="mt-2 grid grid-cols-4 gap-2 sm:grid-cols-5">
                        <button
                            type="button"
                            @click="loginId = null"
                            class="relative flex aspect-video items-center justify-center rounded-[5px] border-2 text-[10px] font-semibold text-gray-400 transition-all duration-150 lg:rounded-[8px]"
                        ...
                        <template x-for="item in library" :key="'login-'+item.id">
                            <button
                                ...
                                class="group relative aspect-video overflow-hidden rounded-[5px] border-2 transition-all duration-150 lg:rounded-[8px]"
        ...
        <div x-data="{ uploadOpen: false, renaming: null, replacing: null }">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="flex gap-1">
                    <a href="{{ route('super-admin.media.index') }}" class="rounded-[5px] px-3 py-1.5 text-sm font-semibold {{ ! request('type') ? 'bg-primary-50 text-primary-600 dark:bg-primary-900/30 dark:text-primary-400' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400' }} lg:rounded-[8px]">All</a>
                    <a href="{{ route('super-admin.media.index', ['type' => 'image']) }}" class="rounded-[5px] px-3 py-1.5 text-sm font-semibold {{ request('type') === 'image' ? 'bg-primary-50 text-primary-600 dark:bg-primary-900/30 dark:text-primary-400' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400' }} lg:rounded-[8px]">Images</a>
                    <a href="{{ route('super-admin.media.index', ['type' => 'video']) }}" class="rounded-[5px] px-3 py-1.5 text-sm font-semibold {{ request('type') === 'video' ? 'bg-primary-50 text-primary-600 dark:bg-primary-900/30 dark:text-primary-400' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400' }} lg:rounded-[8px]">Videos</a>
                </div>

                <button type="button" @click="uploadOpen = true" class="flex items-center gap-2 rounded-[5px] bg-primary-500 px-4 py-2 text-sm font-semibold text-white transition-all duration-300 ease-out hover:-translate-y-0.5 hover:bg-primary-600 hover:shadow-md lg:rounded-[10px]">
            ...
            <div x-show="uploadOpen" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
                <div @click.outside="uploadOpen = false" class="w-full max-w-md rounded-[5px] bg-white p-6 dark:bg-gray-800 lg:rounded-[10px]">
                    ...
                    <form method="POST" action="{{ route('super-admin.media.store') }}" enctype="multipart/form-data" class="mt-4 space-y-4">
                        ...
                        <div class="flex justify-end gap-2">
                            <button type="button" @click="uploadOpen = false" class="rounded-[5px] border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 dark:border-gray-600 dark:text-gray-200 lg:rounded-[10px]">Cancel</button>
                            <button type="submit" class="rounded-[5px] bg-primary-500 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-600 lg:rounded-[10px]">Upload</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div x-show="renaming" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
                <div @click.outside="renaming = null" class="w-full max-w-md rounded-[5px] bg-white p-6 dark:bg-gray-800 lg:rounded-[10px]">
                    ...
                    <form method="POST" :action="renaming ? '{{ route('super-admin.media.update', ['media' => '__ID__']) }}'.replace('__ID__', renaming.id) : '#'" class="mt-4 space-y-4">
                        ...
                        <div class="flex justify-end gap-2">
                            <button type="button" @click="renaming = null" class="rounded-[5px] border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 dark:border-gray-600 dark:text-gray-200 lg:rounded-[10px]">Cancel</button>
            ...
            <div x-show="replacing" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
                <div @click.outside="replacing = null" class="w-full max-w-md rounded-[5px] bg-white p-6 dark:bg-gray-800 lg:rounded-[10px]">
                    ...
                    <form method="POST" :action="replacing ? '{{ route('super-admin.media.replace', ['media' => '__ID__']) }}'.replace('__ID__', replacing.id) : '#'" enctype="multipart/form-data" class="mt-4 space-y-4">
                        ...
                        <input
                            ...
                            required
                            accept=".jpg,.jpeg,.png,.webp,.mp4,.mov,.webm"
                            class="w-full rounded-[5px] border border-gray-300 bg-white py-2.5 px-3 text-sm text-gray-700 shadow-sm file:mr-3 file:rounded-[5px] file:border-0 file:bg-primary-50 file:px-3 file:py-1.5 file:text-sm file:font-semibold file:text-primary-700 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 lg:rounded-[10px]"
                        >
                        <div class="flex justify-end gap-2">
                            <button type="button" @click="replacing = null" class="rounded-[5px] border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 dark:border-gray-600 dark:text-gray-200 lg:rounded-[10px]">Cancel</button>
                            <button type="submit" class="rounded-[5px] bg-primary-500 px-4 py-2 text-sm font-semibold text-white hover:bg-primary-600 lg:rounded-[10px]">Replace</button>
                        </div>
                    </form>
                </div>
            </div>
