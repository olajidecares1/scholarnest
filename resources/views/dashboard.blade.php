<x-dashboard-layout page-title="Dashboard" :page-subtitle="'Welcome back, '.auth()->user()->name.'! Here\'s '.$school->name.' at a glance.'">
            ...
            @switch($accountState)
                ...
                @case('pending')
                    <div class="rounded-[5px] border border-blue-200 bg-blue-50 p-6 text-center dark:border-blue-800 dark:bg-blue-900/20 lg:rounded-[10px]">
                        ...
                        <p class="mt-1 text-sm text-gray-600 dark:text-gray-300">Your account is awaiting payment confirmation and activation by the Super Administrator.</p>
    ...
    @else
        ...
        <div class="grid grid-cols-4 gap-2.5 sm:grid-cols-5 sm:gap-3 md:grid-cols-6 lg:grid-cols-4 lg:gap-4 xl:grid-cols-5">
            ...
            @if ($school->hasPlanAccess(\App\Enums\PlanKey::Basic))
                <x-module-card :href="route('result-pins.index')" label="Result PINs" icon="fa-key" color="text-teal-600 bg-teal-50 dark:bg-teal-900/20" />
            @endif
            ...
            @if (\Illuminate\Support\Facades\Route::has('notices.index'))
                <x-module-card :href="route('notices.index')" label="Notices" icon="fa-bell" color="text-yellow-600 bg-yellow-50 dark:bg-yellow-900/20" />
            @endif
