    ...
    <div class="space-y-6">
        ...
        @if ($years->isEmpty())
            <div class="rounded-[10px] border border-gray-200 bg-white p-10 text-center dark:border-gray-700 dark:bg-gray-800">
                ...
                <small class="block mt-3 text-sm text-gray-500 dark:text-gray-400">No {{ $examBody->name }} past questions are available yet. Check back soon.</small>
            </div>
        @else
            ...
            <div x-data="{ year: {{ $selectedYear }} }" class="space-y-5">
                <div class="rounded-[10px] border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <small class="field-hint mt-1">{{ $years->count() }} year(s) of {{ $examBody->name }} past questions.</small>
                    <div class="mt-3 flex flex-wrap gap-2">
                        @foreach ($years as $year)
                            <button
                                type="button"
                                @click="year = {{ $year }}"
                                class="btn rounded-[8px] border px-4 py-2 text-sm font-bold transition-all duration-200 hover:-translate-y-0.5"
                                :class="year === {{ $year }}
                            ...
                            @foreach ($exams as $exam)
                                <div class="rounded-[10px] border border-gray-200 bg-white p-5 shadow-sm transition-all duration-300 ease-out hover:-translate-y-0.5 hover:shadow-md dark:border-gray-700 dark:bg-gray-800">
                                    ...
                                    <form method="POST" action="{{ route('student.cbt-practice.start', [$school, $exam]) }}" class="mt-4">
                                        @csrf
                                        <button type="submit" class="btn w-full rounded-[8px] bg-primary-600 px-4 py-2 text-sm font-semibold text-white transition-all duration-300 ease-out hover:-translate-y-0.5 hover:bg-primary-700 hover:shadow-md">
