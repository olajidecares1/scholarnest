    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        @forelse ($tests as $test)
            ...
            <div class="rounded-[10px] border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                ...
                @elseif ($myAttempt)
                    <a href="{{ route('student.tests.attempts.show', [$school, $myAttempt]) }}" class="mt-4 inline-flex items-center gap-1.5 rounded-[8px] bg-amber-600 px-4 py-2 text-xs font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-amber-700">
                        Continue Test
                    </a>
                @elseif ($test->isOpenForStudents())
                    <form method="POST" action="{{ route('student.tests.start', [$school, $test]) }}" class="mt-4">
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-1.5 rounded-[8px] bg-primary-600 px-4 py-2 text-xs font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary-700">
                            Start Test
                        </button>
                    </form>
                @else
        ...
        @empty
            <p class="col-span-full py-10 text-center text-sm text-gray-500 dark:text-gray-400">No tests assigned yet.</p>
        @endforelse
    </div>
</x-student-layout>
