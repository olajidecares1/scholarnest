    ...
    <div class="space-y-4 sm:space-y-6">
            ...
            <a href="{{ route('staff.attendance.index', $school) }}" class="flex items-center justify-between gap-3 rounded-[10px] border border-green-200 bg-green-50 p-4 shadow-sm sm:p-5 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md dark:border-green-900/40 dark:bg-green-900/10">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <small class="field-hint">You're the Class Teacher of {{ $classTeacherOf->implode(', ') }}.</small>
                    </div>
                </div>
        ...
        @if ($staff->role === \App\Enums\StaffRole::Teacher && ($classTeacherOf->isNotEmpty() || $subjectAssignments->isNotEmpty()))
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div class="rounded-[10px] border border-gray-200 bg-white p-4 shadow-sm sm:p-5 dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <small class="field-hint mt-1">Classes you're the Class Teacher of.</small>
                    <div class="mt-3 flex flex-wrap gap-2">
                        ...
                        @empty
                            <small class="field-hint">Not assigned to any class yet.</small>
                        @endforelse
                    </div>
                </div>
                <div class="rounded-[10px] border border-gray-200 bg-white p-4 shadow-sm sm:p-5 dark:border-gray-700 dark:bg-gray-800">
                    <h2 class="text-sm font-bold text-gray-900 dark:text-white">My Subjects</h2>
                    <small class="field-hint mt-1">Subjects you've been assigned to teach.</small>
                    <div class="mt-3 space-y-1.5">
                        ...
                        @empty
                            <small class="field-hint">Not assigned to any subject yet.</small>
                        @endforelse
                    </div>
                </div>
            </div>
        ...
        <div class="rounded-[10px] border border-gray-200 bg-white p-4 shadow-sm sm:p-5 dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="mt-4 space-y-3">
                ...
                @empty
                    <small class="block rounded-[8px] border border-dashed border-gray-200 py-8 text-center text-sm text-gray-500 dark:border-gray-700 dark:text-gray-400">No classes scheduled for today.</small>
                @endforelse
            </div>
        </div>
    </div>
