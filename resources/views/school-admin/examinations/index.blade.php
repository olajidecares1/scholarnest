    ...
    <div class="space-y-6" x-data="{ open: false }">
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white shadow-sm lg:rounded-[10px]">
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 p-6">
                <form method="GET" class="flex flex-wrap items-end gap-2">
                    ...
                    <button type="submit" class="h-11 rounded-[8px] border border-gray-300 px-4 text-sm font-semibold text-gray-700 transition-all duration-200 hover:bg-gray-50">Filter</button>
                    @if (request('class') || request('term'))
                        <a href="{{ route('examinations.index') }}" class="text-sm font-semibold text-blue-600 hover:text-blue-700">Clear</a>
                    @endif
                </form>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($examinations as $examination)
                            <tr class="transition-colors duration-200 hover:bg-gray-50">
                                <td class="px-6 py-3">
                                    <a href="{{ route('examinations.show', $examination) }}" class="font-semibold text-gray-900 hover:text-blue-600">{{ $examination->name }}</a>
                                </td>
                                <td class="px-6 py-3 text-gray-600">{{ $examination->class_name }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $examination->term->label() }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $examination->session }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $examination->subjects_count }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $examination->exam_date?->format('M j, Y') ?? '—' }}</td>
                                <td class="px-6 py-3">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('examinations.show', $examination) }}" class="rounded-[8px] border border-gray-300 px-3 py-1.5 text-xs font-semibold text-gray-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-gray-50">Manage</a>
                                        <a href="{{ route('examinations.report-cards.index', $examination) }}" class="rounded-[8px] border border-gray-300 px-3 py-1.5 text-xs font-semibold text-gray-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-gray-50">Report Cards</a>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-10 text-center text-sm text-gray-500">No examinations yet. Click "New Examination" to create one.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="open" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
            <div @click.outside="open = false" class="w-full max-w-lg rounded-[8px] bg-white p-6">
                <h3 class="text-sm font-bold text-gray-900">New Examination</h3>
