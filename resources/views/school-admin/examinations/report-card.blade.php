...
<x-dashboard-layout :page-title="$student->fullName().' — Report Card'" :page-subtitle="$examination->name.' · '.$examination->class_name">
    <div class="space-y-6">
        <a href="{{ route('examinations.report-cards.index', $examination) }}" class="inline-flex items-center gap-1.5 text-sm font-semibold text-blue-600 transition-colors duration-150 hover:text-blue-700">
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white p-6 shadow-sm lg:rounded-[10px]">
            <div class="flex flex-wrap items-center gap-4">
                ...
                @else
                    <span class="flex h-16 w-16 items-center justify-center rounded-full bg-blue-100 text-xl font-bold text-blue-700">
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white shadow-sm lg:rounded-[10px]">
            <div class="border-b border-gray-100 p-6">
                <h3 class="text-sm font-bold text-gray-900">Subject Scores</h3>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($subjects as $subject)
                            ...
                            <tr class="transition-colors duration-200 hover:bg-gray-50">
                                ...
                                <td class="px-6 py-3 text-gray-600">{{ $score ? $score->percentage().'%' : '—' }}</td>
                                <td class="px-6 py-3">
                                    @if ($score)
                                        <span class="rounded-full bg-blue-100 px-2 py-0.5 text-xs font-semibold text-blue-700">{{ $score->grade() }}</span>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center text-sm text-gray-500">No subjects have been added to this examination yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
