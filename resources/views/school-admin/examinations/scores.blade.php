    ...
    <div class="space-y-6">
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white shadow-sm lg:rounded-[10px]">
            <div class="border-b border-gray-100 p-6">
                <h2 class="text-sm font-bold text-gray-900">{{ $subject->name }} &middot; Max Score {{ $subject->max_score }}</h2>
                <p class="mt-0.5 text-xs text-gray-500">Enter each student's score out of {{ $subject->max_score }}. Leave blank to skip a student.</p>
            </div>
                                ...
                                <tr class="transition-colors duration-200 hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-3">
                                        <input
                                            ...
                                            step="0.01"
                                            class="h-10 w-28 rounded-[8px] border border-gray-300 px-3 text-sm text-gray-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-[3px] focus:ring-blue-500/15"
                                        >
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="2" class="px-6 py-10 text-center text-sm text-gray-500">No active students in {{ $examination->class_name }}.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
