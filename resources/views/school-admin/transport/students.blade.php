    ...
    <div class="space-y-6" x-data="{ open: false }">
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white shadow-sm lg:rounded-[10px]">
            <div class="flex items-center justify-between border-b border-gray-100 p-6">
                <div>
                    <h2 class="text-sm font-bold text-gray-900">{{ $route->name }}</h2>
                    <p class="mt-0.5 text-xs text-gray-500">{{ $assignments->count() }} student(s) assigned</p>
                </div>
                <button
                    type="button"
                    @click="open = true"
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($assignments as $assignment)
                            <tr class="transition-colors duration-200 hover:bg-gray-50">
                                <td class="px-6 py-3 font-semibold text-gray-900">{{ $assignment->student->fullName() }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $assignment->student->class_name ?? '—' }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $assignment->pickup_point ?? '—' }}</td>
                                <td class="px-6 py-3">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center text-sm text-gray-500">No students assigned to this route yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="open" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
            <div @click.outside="open = false" class="w-full max-w-md rounded-[8px] bg-white p-6">
                <h3 class="text-sm font-bold text-gray-900">Assign Student</h3>
