    ...
    <div class="space-y-6" x-data="{ open: false }">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex gap-2 rounded-[8px] border border-gray-200 bg-white p-1">
                <a href="{{ route('library.index') }}" class="rounded-[6px] px-4 py-1.5 text-sm font-semibold text-gray-600 transition-colors duration-150 hover:bg-gray-50">Catalog</a>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 p-6">
                <div class="flex gap-1 rounded-[8px] border border-gray-200 bg-gray-50 p-1">
                    @foreach (['active' => 'Active', 'overdue' => 'Overdue', 'returned' => 'Returned', 'all' => 'All'] as $value => $label)
                        <a href="{{ route('library.loans.index', ['status' => $value]) }}" class="rounded-[6px] px-3 py-1.5 text-xs font-semibold transition-colors duration-150 {{ $status === $value ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100' }}">{{ $label }}</a>
                    @endforeach
                </div>
                <form method="GET" class="flex items-center gap-2">
                    ...
                    <input
                        ...
                        class="h-10 w-56 rounded-[8px] border border-gray-300 px-3 text-sm text-gray-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-[3px] focus:ring-blue-500/15"
                    >
                </form>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($loans as $loan)
                            <tr class="transition-colors duration-200 hover:bg-gray-50">
                                <td class="px-6 py-3 font-semibold text-gray-900">{{ $loan->book->title }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $loan->student->fullName() }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $loan->borrowed_at->format('M j, Y') }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $loan->due_at->format('M j, Y') }}</td>
                                <td class="px-6 py-3">
                                ...
                                <td class="px-6 py-3">
                                    @unless ($loan->isReturned())
                                        <form method="POST" action="{{ route('library.loans.return', $loan) }}">
                                            @csrf
                                            <button type="submit" class="rounded-[8px] border border-gray-300 px-3 py-1.5 text-xs font-semibold text-gray-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-gray-50">Mark Returned</button>
                                        </form>
                                    @else
                                        <span class="text-xs text-gray-400">Returned {{ $loan->returned_at->format('M j, Y') }}</span>
                                    @endunless
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-10 text-center text-sm text-gray-500">No {{ $status === 'all' ? '' : $status.' ' }}loans found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="open" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
            <div @click.outside="open = false" class="w-full max-w-lg rounded-[8px] bg-white p-6">
                <h3 class="text-sm font-bold text-gray-900">Issue Book</h3>
