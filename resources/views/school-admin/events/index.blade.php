    ...
    <div class="space-y-6" x-data="{ open: false, editing: null }">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-[5px] border border-gray-200 bg-white p-5 shadow-sm lg:rounded-[10px]">
                <p class="text-sm font-medium text-purple-600">Upcoming Events</p>
                <p class="mt-2 text-3xl font-extrabold text-gray-900">{{ number_format($upcomingCount) }}</p>
            </div>
        </div>
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <div class="flex gap-1 rounded-[8px] border border-gray-200 bg-gray-50 p-1">
                        @foreach (['upcoming' => 'Upcoming', 'past' => 'Past', 'all' => 'All'] as $value => $label)
                            <a href="{{ route('events.index', ['when' => $value]) }}" class="rounded-[6px] px-3 py-1.5 text-xs font-semibold transition-colors duration-150 {{ $when === $value ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100' }}">{{ $label }}</a>
                        @endforeach
                    </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="transition-colors duration-200 hover:bg-gray-50">
                                <td class="px-6 py-3">
                                    ...
                                    @if ($event->description)
                                        <p class="mt-0.5 max-w-xs truncate text-xs text-gray-500">{{ $event->description }}</p>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-3 text-gray-600">{{ $event->location ?? '—' }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $event->audience->label() }}</td>
                                <td class="px-6 py-3">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-10 text-center text-sm text-gray-500">No {{ $when === 'all' ? '' : $when.' ' }}events found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="open" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
            <div @click.outside="open = false" class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-[8px] bg-white p-6">
                <h3 class="text-sm font-bold text-gray-900" x-text="editing ? 'Edit Event' : 'New Event'"></h3>
                <form
                    method="POST"
                ...
                >
                    ...
                    <label class="flex items-center gap-2 text-sm text-gray-700">
                        <input type="checkbox" name="is_all_day" value="1" :checked="editing ? editing.is_all_day : false" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                        All-day event
                    </label>
