    ...
    <div class="space-y-6">
            ...
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    ...
                    @if ($assignment->description)
                        <p class="mt-3 text-sm text-gray-600">{{ $assignment->description }}</p>
                    @endif
                </div>
                <form method="POST" action="{{ route('assignments.destroy', $assignment) }}" onsubmit="return confirm('Delete {{ $assignment->title }}? All submissions will be removed.');">
                    @csrf @method('DELETE')
                    <button type="submit" class="rounded-[8px] border border-red-300 px-3 py-1.5 text-xs font-semibold text-red-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-red-50">Delete Assignment</button>
                </form>
            </div>
        </div>
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white shadow-sm lg:rounded-[10px]">
            <div class="border-b border-gray-100 p-6">
                <h3 class="text-sm font-bold text-gray-900">Student Submissions</h3>
                <p class="mt-0.5 text-xs text-gray-500">Track submission status and record scores per student.</p>
            </div>
            ...
            <form method="POST" action="{{ route('assignments.submissions.store', $assignment) }}">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="transition-colors duration-200 hover:bg-gray-50">
                                        ...
                                        <div class="grid max-w-sm grid-cols-4 gap-1.5">
                                            @foreach ($statusOptions as $status)
                                                <label class="cursor-pointer">
                                                    ...
                                                    <span class="block rounded-[6px] border border-gray-300 px-1.5 py-1.5 text-center text-[11px] font-semibold text-gray-500 transition-colors duration-150 peer-checked:border-transparent peer-checked:{{ $status->badgeClasses() }}">
                                                        {{ $status->label() }}
                                                    </span>
                                                </label>
                                            @endforeach
                                    ...
                                    <td class="px-6 py-3">
                                        <input
                                            ...
                                            step="0.01"
                                            class="h-10 w-24 rounded-[8px] border border-gray-300 px-3 text-sm text-gray-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-[3px] focus:ring-blue-500/15"
                                        >
                                    </td>
                                    <td class="px-6 py-3">
                                        <input
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-10 text-center text-sm text-gray-500">No active students in {{ $assignment->class_name }}.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                @if ($students->isNotEmpty())
                    <div class="flex justify-end border-t border-gray-100 p-6">
