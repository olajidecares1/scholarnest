    ...
    <div class="space-y-6" x-data="{ open: false }">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 p-6">
                <form method="GET" class="flex flex-wrap items-end gap-2">
                    ...
                    <input
                        ...
                        class="h-11 w-48 rounded-[8px] border border-gray-300 px-3 text-sm text-gray-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-[3px] focus:ring-blue-500/15"
                    >
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($assignments as $assignment)
                            <tr class="transition-colors duration-200 hover:bg-gray-50">
                                <td class="px-6 py-3">
                                    <a href="{{ route('assignments.show', $assignment) }}" class="font-semibold text-gray-900 hover:text-blue-600">{{ $assignment->title }}</a>
                                </td>
                                <td class="px-6 py-3 text-gray-600">{{ $assignment->class_name }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $assignment->subject }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $assignment->due_date->format('M j, Y') }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $assignment->graded_count }}</td>
                                <td class="px-6 py-3">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('assignments.show', $assignment) }}" class="rounded-[8px] border border-gray-300 px-3 py-1.5 text-xs font-semibold text-gray-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-gray-50">Manage</a>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-10 text-center text-sm text-gray-500">No assignments yet. Click "New Assignment" to create one.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="open" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
            <div @click.outside="open = false" class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-[8px] bg-white p-6">
                <h3 class="text-sm font-bold text-gray-900">New Assignment</h3>
