    ...
    <div class="space-y-6" x-data="{ open: false }">
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white shadow-sm lg:rounded-[10px]">
            <div class="flex items-center justify-between border-b border-gray-100 p-6">
                <div>
                    <h2 class="text-sm font-bold text-gray-900">Room {{ $room->room_number }}</h2>
                    <p class="mt-0.5 text-xs text-gray-500">{{ $allocations->count() }} / {{ $room->capacity }} beds occupied</p>
                </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($allocations as $allocation)
                            <tr class="transition-colors duration-200 hover:bg-gray-50">
                                <td class="px-6 py-3 font-semibold text-gray-900">{{ $allocation->student->fullName() }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $allocation->student->class_name ?? '—' }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $allocation->allocated_date->format('M j, Y') }}</td>
                                <td class="px-6 py-3">
                                    <form method="POST" action="{{ route('hostels.allocations.destroy', $allocation) }}" onsubmit="return confirm('Vacate {{ $allocation->student->fullName() }} from this room?');">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="rounded-[8px] border border-red-300 px-3 py-1.5 text-xs font-semibold text-red-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-red-50">Vacate</button>
                                    </form>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center text-sm text-gray-500">No students allocated to this room yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="open" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
            <div @click.outside="open = false" class="w-full max-w-md rounded-[8px] bg-white p-6">
                <h3 class="text-sm font-bold text-gray-900">Allocate Student</h3>
