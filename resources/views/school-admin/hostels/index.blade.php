    ...
    <div class="space-y-6" x-data="{ open: false, editing: null }">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($hostels as $hostel)
                            <tr class="transition-colors duration-200 hover:bg-gray-50">
                                <td class="px-6 py-3 font-semibold text-gray-900">{{ $hostel->name }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $hostel->gender->label() }}</td>
                                <td class="px-6 py-3 text-gray-600">
                                    {{ $hostel->warden_name ?? '—' }}
                                    @if ($hostel->warden_phone) <span class="text-gray-400">&middot; {{ $hostel->warden_phone }}</span> @endif
                                </td>
                                <td class="px-6 py-3 text-gray-600">{{ $hostel->rooms_count }}</td>
                                <td class="px-6 py-3">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('hostels.rooms', $hostel) }}" class="rounded-[8px] border border-gray-300 px-3 py-1.5 text-xs font-semibold text-gray-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-gray-50">Manage Rooms</a>
                                        <button
                                            type="button"
                                            @click="editing = @js([
                                                'uuid' => $hostel->uuid,
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-10 text-center text-sm text-gray-500">No hostels yet. Click "Add Hostel" to create one.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="open" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
            <div @click.outside="open = false" class="w-full max-w-lg rounded-[8px] bg-white p-6">
                <h3 class="text-sm font-bold text-gray-900" x-text="editing ? 'Edit Hostel' : 'Add Hostel'"></h3>
                <form
                    method="POST"
