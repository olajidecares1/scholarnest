    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex gap-2 rounded-[8px] border border-gray-200 bg-white p-1">
                ...
                <a href="{{ route('attendance.history') }}" class="rounded-[6px] px-4 py-1.5 text-sm font-semibold text-gray-600 transition-colors duration-150 hover:bg-gray-50">History</a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div class="rounded-[5px] border border-gray-200 bg-white p-5 shadow-sm lg:rounded-[10px]">
                ...
                <p class="mt-2 text-3xl font-extrabold text-gray-900">{{ number_format($students->count()) }}</p>
            </div>
            <div class="rounded-[5px] border border-gray-200 bg-white p-5 shadow-sm lg:rounded-[10px]">
                <p class="text-sm font-medium text-blue-600">Marked for {{ $date->format('M j') }}</p>
                <p class="mt-2 text-3xl font-extrabold text-gray-900">{{ number_format($markedCount) }}</p>
            </div>
            <div class="rounded-[5px] border border-gray-200 bg-white p-5 shadow-sm lg:rounded-[10px]">
                <p class="text-sm font-medium text-green-600">Present / Late</p>
                <p class="mt-2 text-3xl font-extrabold text-gray-900">{{ number_format($presentCount) }}</p>
            </div>
        </div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 p-6">
                <form method="GET" class="flex flex-wrap items-end gap-2">
                    <input
                        ...
                        class="h-11 rounded-[8px] border border-gray-300 px-3 text-sm text-gray-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-[3px] focus:ring-blue-500/15"
                    >
                    <div
                        class="w-48"
                                        ...
                                        <div class="grid max-w-md grid-cols-4 gap-1.5">
                                            @foreach ($statusOptions as $status)
                                                <label class="cursor-pointer">
                                                    ...
                                                    <span class="block rounded-[6px] border border-gray-300 px-2 py-1.5 text-center text-xs font-semibold text-gray-500 transition-colors duration-150 peer-checked:border-transparent peer-checked:{{ $status->badgeClasses() }}">
                                                        {{ $status->label() }}
                                                    </span>
                                                </label>
                                            @endforeach
