<x-dashboard-layout page-title="Academics" page-subtitle="Organize your school into academic levels and classes.">
    <div class="space-y-6" x-data="{ addLevelOpen: false }">
            ...
            @forelse ($levels as $level)
                ...
                >
                    <div class="flex items-center justify-between gap-2">
                        ...
                        >
                            @csrf @method('PUT')
                            <input type="text" name="name" x-model="levelName" class="w-full rounded-[8px] border border-gray-300 px-2 py-1 text-sm font-bold text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/10">
                        ...
                        <div class="flex shrink-0 items-center gap-1">
                            <button type="button" @click="editingLevel = !editingLevel" class="rounded-[8px] p-1.5 text-gray-400 transition-colors duration-150 hover:bg-gray-100 hover:text-blue-600">
                            ...
                            <form method="POST" action="{{ route('academics.levels.destroy', $level) }}" onsubmit="return confirm('Delete {{ $level->name }} and every class inside it?');">
                                @csrf @method('DELETE')
                                <button type="submit" class="rounded-[8px] p-1.5 text-gray-400 transition-colors duration-150 hover:bg-red-50 hover:text-red-600">
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        @forelse ($level->classes as $class)
                            <span class="group inline-flex items-center gap-2 rounded-[8px] border border-gray-200 bg-gray-50 py-1.5 pl-3 pr-1.5 text-sm text-gray-700">
                                ...
                                <form method="POST" action="{{ route('academics.classes.destroy', $class) }}" onsubmit="return confirm('Delete {{ $class->name }}?');">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="rounded-[8px] p-1 text-gray-400 transition-colors duration-150 hover:bg-red-50 hover:text-red-600">
                    ...
                    >
                        @csrf
                        <input type="text" name="name" placeholder="e.g. JSS 1" required class="w-full rounded-[8px] border border-gray-300 px-2 py-1.5 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/10">
                        <button type="submit" class="shrink-0 rounded-[8px] bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700">Add</button>
                    </form>
                </div>
            @empty
        ...
        <div x-show="addLevelOpen" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
            <div @click.outside="addLevelOpen = false" class="w-full max-w-md rounded-[8px] bg-white p-6">
                ...
                <form method="POST" action="{{ route('academics.levels.store') }}" class="mt-4 space-y-4">
                    @csrf
                    <input type="text" name="name" required placeholder="Level name" class="w-full rounded-[8px] border border-gray-300 px-3 py-2.5 text-sm font-medium text-gray-900 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/10">
                    <div class="flex justify-end gap-2">
                        <button type="button" @click="addLevelOpen = false" class="rounded-[8px] border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700">Cancel</button>
