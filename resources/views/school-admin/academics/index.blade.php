                ...
                >
                    <div class="flex items-center justify-between gap-2">
                        <template x-if="!editingLevel">
                            <h2 class="text-sm font-bold text-gray-900 dark:text-white" x-text="levelName"></h2>
                        </template>
                        <form
                            x-show="editingLevel"
                            method="POST"
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        @forelse ($level->classes as $class)
                            <span class="group inline-flex items-center gap-2 rounded-[8px] border border-gray-200 bg-gray-50 py-1.5 pl-3 pr-1.5 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-700 dark:text-gray-200">
                                ...
                                <form method="POST" action="{{ route('academics.classes.destroy', $class) }}" onsubmit="return confirm('Delete {{ $class->name }}?');">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="rounded-[8px] p-1 text-gray-400 transition-colors duration-150 hover:bg-red-50 hover:text-red-600 dark:text-gray-500 dark:hover:bg-red-900/20 dark:hover:text-red-400">
                        ...
                        @empty
                            <p class="text-xs text-gray-400 dark:text-gray-500">No classes in this level yet.</p>
                        @endforelse
                    </div>
                    ...
                    >
                        @csrf
                        <input type="text" name="name" placeholder="e.g. JSS 1" required class="w-full rounded-[8px] border border-gray-300 px-2 py-1.5 text-sm text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/10 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
