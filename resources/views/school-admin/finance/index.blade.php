    ...
    <div class="space-y-6" x-data="{ open: false }">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex gap-2 rounded-[8px] border border-gray-200 bg-white p-1">
                ...
                <a href="{{ route('finance.invoices.index') }}" class="rounded-[6px] px-4 py-1.5 text-sm font-semibold text-gray-600 transition-colors duration-150 hover:bg-gray-50">Invoices</a>
            </div>
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white shadow-sm lg:rounded-[10px]">
            <div class="border-b border-gray-100 p-6">
                <h2 class="text-sm font-bold text-gray-900">Fee Structures</h2>
                <p class="mt-0.5 text-xs text-gray-500">Define a fee once, then generate an invoice for every matching student.</p>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($structures as $structure)
                            <tr class="transition-colors duration-200 hover:bg-gray-50">
                                <td class="px-6 py-3 font-semibold text-gray-900">{{ $structure->name }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $structure->class_name ?? 'All Classes' }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $structure->session }} @if ($structure->term) &middot; {{ $structure->term }} @endif</td>
                                <td class="px-6 py-3 font-semibold text-gray-900">&#8358;{{ number_format((float) $structure->amount, 2) }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $structure->invoices_count }}</td>
                                <td class="px-6 py-3">
                                    <div class="flex items-center gap-2">
                                        <form method="POST" action="{{ route('finance.generate', $structure) }}" onsubmit="return confirm('Generate invoices for every matching active student who doesn\'t already have one?');">
                                            @csrf
                                            <button type="submit" class="rounded-[8px] border border-gray-300 px-3 py-1.5 text-xs font-semibold text-gray-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-gray-50">Generate Invoices</button>
                                        </form>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-10 text-center text-sm text-gray-500">No fee structures yet. Click "New Fee Structure" to define one.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="open" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
            <div @click.outside="open = false" class="w-full max-w-lg rounded-[8px] bg-white p-6">
                <h3 class="text-sm font-bold text-gray-900">New Fee Structure</h3>
