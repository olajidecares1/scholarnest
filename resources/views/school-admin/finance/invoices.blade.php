    ...
    <div class="space-y-6" x-data="{ addOpen: false, payOpen: false, paying: null }">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="flex gap-2 rounded-[8px] border border-gray-200 bg-white p-1">
                <a href="{{ route('finance.index') }}" class="rounded-[6px] px-4 py-1.5 text-sm font-semibold text-gray-600 transition-colors duration-150 hover:bg-gray-50">Fee Structures</a>
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white p-5 shadow-sm lg:rounded-[10px]">
            ...
            <p class="mt-2 text-3xl font-extrabold text-gray-900">&#8358;{{ number_format($totalOutstanding, 2) }}</p>
        </div>
        ...
        <div class="rounded-[5px] border border-gray-200 bg-white shadow-sm lg:rounded-[10px]">
            <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 p-6">
                <div class="flex gap-1 rounded-[8px] border border-gray-200 bg-gray-50 p-1">
                    @foreach (['' => 'All', 'unpaid' => 'Unpaid', 'partial' => 'Partial', 'paid' => 'Paid'] as $value => $label)
                        <a href="{{ route('finance.invoices.index', ['status' => $value]) }}" class="rounded-[6px] px-3 py-1.5 text-xs font-semibold transition-colors duration-150 {{ request('status', '') === $value ? 'bg-blue-600 text-white' : 'text-gray-600 hover:bg-gray-100' }}">{{ $label }}</a>
                    @endforeach
                </div>
                <form method="GET" class="flex flex-wrap items-end gap-2">
                    ...
                    <input
                        ...
                        class="h-10 w-52 rounded-[8px] border border-gray-300 px-3 text-sm text-gray-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-[3px] focus:ring-blue-500/15"
                    >
                    <div class="w-40">
                        <x-select-field name="class" placeholder="All Classes" :selected="request('class')" :options="['' => 'All Classes'] + $classOptions" />
                    </div>
                    <button type="submit" class="h-10 rounded-[8px] border border-gray-300 px-4 text-sm font-semibold text-gray-700 transition-all duration-200 hover:bg-gray-50">Filter</button>
                </form>
            </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($invoices as $invoice)
                            <tr class="transition-colors duration-200 hover:bg-gray-50">
                                <td class="px-6 py-3 font-semibold text-gray-900">{{ $invoice->student->fullName() }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $invoice->title }}</td>
                                <td class="px-6 py-3 text-gray-600">&#8358;{{ number_format((float) $invoice->amount, 2) }}</td>
                                <td class="px-6 py-3 text-gray-600">&#8358;{{ number_format($invoice->amountPaid(), 2) }}</td>
                                <td class="px-6 py-3 font-semibold text-gray-900">&#8358;{{ number_format($invoice->balance(), 2) }}</td>
                                <td class="px-6 py-3">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-10 text-center text-sm text-gray-500">No invoices found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="addOpen" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
            <div @click.outside="addOpen = false" class="w-full max-w-lg rounded-[8px] bg-white p-6">
                <h3 class="text-sm font-bold text-gray-900">Add Invoice</h3>
                <form method="POST" action="{{ route('finance.invoices.store') }}" class="mt-4 space-y-4">
                    ...
                    <div class="flex justify-end gap-2">
                        <button type="button" @click="addOpen = false" class="rounded-[8px] border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700">Cancel</button>
        ...
        <div x-show="payOpen" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 p-4">
            <div @click.outside="payOpen = false" class="w-full max-w-md rounded-[8px] bg-white p-6">
                <h3 class="text-sm font-bold text-gray-900">Record Payment</h3>
                <p class="mt-1 text-xs text-gray-500" x-show="paying">
                ...
                >
                    ...
                    <div class="flex justify-end gap-2">
                        <button type="button" @click="payOpen = false" class="rounded-[8px] border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700">Cancel</button>
