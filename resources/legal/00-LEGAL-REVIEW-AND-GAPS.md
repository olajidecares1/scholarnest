...
3. ~~Cross-border transfer of payment receipts.~~ Resolved: receipts are now checked on AkademicNest's own servers.
...
9. No processing agreement with schools.
...
| 6 | ~~Receipt screening transfer~~ Resolved: screening now runs locally | |
...
### GAP-04 · Cross-border transfer of receipts, **RESOLVED**

`PaymentReceiptScreening` now reads receipts on AkademicNest's own servers: PDF
text is extracted locally and images are checked for being readable. No receipt
leaves the platform, so no transfer mechanism or processor agreement is needed.
...
3. **Controller/processor allocation**: the mapping in the Framework is
   ...
   the arguable edges.
4. **Processor obligations and DPA**: none exists with schools. Normally a
   legal requirement, not good practice.
5. **International transfers**: whether font requests count; where hosting
   actually sits.
