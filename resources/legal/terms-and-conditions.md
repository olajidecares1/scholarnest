...
| **School Content** | Information a School or its users enter or upload — pupil records, staff records, results, attendance, photographs, signatures, news, events, gallery images, website text and similar. |
| **Subscription** | A School's paid right to use the Platform on a given plan for a given period. |

## 3. Acceptance of Terms

Acceptance happens when a School Administrator ticks the agreement box during
registration, and again each time any user signs in.
...
section 25 and the *School Data Processing Framework*.
...
are used under their own licences.
...
This right is a right of **access**, not of ownership, and it ends when the
Subscription ends.
...
- use the account only for the purpose their School gave it to them for;
- not attempt to reach records belonging to another school, another pupil or
  another member of staff;
- report anything that looks like a security problem rather than exploring it.

Accounts created by a School with a temporary password must be given a new
password by the user before the portal can be used for anything else.
...
8. upload anything unlawful, or any file intended to damage or gain
...
results, result tokens, ID cards, facilities and the school's settings. Priced
per pupil per term.

### Standard Plan
Everything in Basic, plus the modules gated to Standard and above: CBT and CBT
practice, parent/guardian accounts, events, news, careers, testimonials,
co-curricular activities, the public school website, assignments, library,
transport, hostel, finance, teacher diary and timetable. Priced per pupil per
term.

### Exclusive Plan
Everything in Standard, plus connecting the School's own custom domain.

> **Exclusive is not currently available for purchase.** The plan is built and
> its features work, but the Platform does not presently accept new
> subscriptions to it. Custom domains are therefore not obtainable at this time.
...
Because receipts are reviewed by hand, activation is not instant. We do not
commit to a specific turnaround time in these Terms.
...
- Capacity counts **active** pupils. Deactivating a pupil who has left releases
  their place.
- The Platform will refuse to create or reactivate a pupil beyond the School's
  capacity. This is enforced under a database lock, so it cannot be bypassed by
  submitting twice.
- Exclusive is not sold per pupil and is not capped in this way.

## 14. Additional pupil capacity

A School may buy more places at any time by submitting a top-up with proof of
payment. Approved top-ups **add to** the existing allocation; they do not reset
or replace it. Top-ups follow the same manual review as the original payment.
...
Deletion Policy*. **A School should export or otherwise secure anything it needs
to keep before asking for deletion**, and should note the limitation described
in section 35.
...
The Platform lets an authorised signer — a School Administrator or a member of
staff — draw a signature, which is stored as an image and printed on documents
such as report cards.

- A signature is always recorded against **the account that drew it**, taken
  from the signed-in session. It cannot be attributed to somebody else by
  altering a form.
- One signer has one signature; drawing a new one replaces the old.
...
**Note.** Signature images are currently stored where they can be retrieved by
anyone holding the file's address. See the *Security & Data Handling Statement*.
...
Schools on Standard and above get a public website within their Environment.
Its content — text, news, events, images, contact details, branding — is chosen
...
- A submission is routed to **the school whose page it was sent from**, taken
  from the address of the page. A sender cannot direct it elsewhere.
...
The Cookie & Browser Storage Policy lists what the Platform stores in a
browser. Signing in requires a session cookie; it cannot be refused without
making the Platform unusable.

## 27. Security

The *Security & Data Handling Statement* describes the protections actually
implemented, and states plainly where protection is weaker than it should be.
No system is perfectly secure and we do not claim otherwise.

A School must keep its own credentials safe. Most incidents in systems like this
begin with a shared or reused password rather than with a flaw in the software.
...
We may take the Platform down for maintenance. Where work is planned and likely
to be disruptive, we will try to give notice and to choose a quiet time. Urgent
work — particularly security work — may happen without notice.
...
applicable law does not permit to be excluded.** Where a mandatory statutory
right applies, it applies.
...
does not permit to be limited or excluded** — which, depending on jurisdiction,
may include certain liabilities under data protection law.
...
need to be kept for longer than a Subscription lasts — a school leaver may
...
We may add, change or withdraw features. Where a change materially reduces
what a School has paid for, we will give reasonable notice and, where
appropriate, a pro-rata refund of the affected period.
...
> Terms a School accepted, or when. Until it does, proving acceptance of any
> particular version will be difficult. See the gap report.
<!-- internal:end -->

## 39. Governing law

These Terms are governed by the laws of [TO BE PROVIDED — expected: the Federal
Republic of Nigeria].

## 40. Dispute resolution

The parties will first attempt to resolve any dispute in good faith by
discussion. Failing that: [TO BE PROVIDED — courts of a named jurisdiction, or
an arbitration clause naming seat, rules and language].

Nothing here prevents either party from seeking urgent injunctive relief, or a
user from complaining to a data protection authority.
...
- **Entire agreement.** These Terms, with the policies they reference, are the
  whole agreement between the parties on their subject.
